...
fn table(c: &mut Criterion) {
    ...
    for a in inputs.take(10) {
        ...
        group.bench_with_input(
            BenchmarkId::new("factor_chunk_false", &a_str),
            &a,
            |b, &a| {
                b.iter(|| {
                    let mut n_s = a;
                    let mut f_s: [_; INPUT_SIZE] = array_init(|_| Factors::one(false));
        ...
        group.bench_with_input(BenchmarkId::new("factor_false", &a_str), &a, |b, &a| {
            b.iter(|| {
                let mut n_s = a;
                let mut f_s: [_; INPUT_SIZE] = array_init(|_| Factors::one(false));
        ...
        group.bench_with_input(
            BenchmarkId::new("factor_chunk_true", &a_str),
            &a,
            |b, &a| {
                b.iter(|| {
                    let mut n_s = a;
                    let mut f_s: [_; INPUT_SIZE] = array_init(|_| Factors::one(true));
                    for (n_s, f_s) in n_s.chunks_mut(CHUNK_SIZE).zip(f_s.chunks_mut(CHUNK_SIZE)) {
                        factor_chunk(n_s.try_into().unwrap(), f_s.try_into().unwrap());
                    }
                });
        ...
        group.bench_with_input(BenchmarkId::new("factor_true", &a_str), &a, |b, &a| {
            b.iter(|| {
                let mut n_s = a;
                let mut f_s: [_; INPUT_SIZE] = array_init(|_| Factors::one(true));
                for (n, f) in n_s.iter_mut().zip(f_s.iter_mut()) {
